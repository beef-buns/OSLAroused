...
void Papyrus::RegisterSceneStart(RE::StaticFunctionTag *, bool bIsOstim, int sceneId,
                                 RE::reference_array<RE::Actor *> actorRefs) {
...
void Papyrus::RemoveScene(RE::StaticFunctionTag *, bool bIsOstim, int sceneId) {
    SceneManager::GetSingleton()->RemoveScene(
            bIsOstim ? SceneManager::SceneFramework::kOStim : SceneManager::SceneFramework::kSexLab, sceneId);
}

void Papyrus::RegisterActorOrgasm(RE::StaticFunctionTag *, RE::Actor *actorRef) {
    PersistedData::LastOrgasmTimeData::GetSingleton()->SetData(actorRef->formID,
...
bool Papyrus::FormHasKeywordString(RE::StaticFunctionTag *, RE::TESForm *form, RE::BSFixedString keyword) {
    ...
    auto *keywords = form->As<RE::BGSKeywordForm>();
    if (!keywords) {
...
std::vector<RE::BSFixedString> Papyrus::GetRegisteredKeywords(RE::StaticFunctionTag *base) {
    ...
    for (auto &keyword: Config::GetSingleton()->GetRegisteredKeywords()) {
        registeredKeywords.emplace_back(keyword.first);
    }
...
void Papyrus::Events::SendActorArousalUpdatedEvent(RE::Actor *actorRef, float newArousal) {
    SendModEvent("OSLA_ActorArousalUpdated", newArousal, actorRef);
}
