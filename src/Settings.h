...
public:
    ...
    static void Load() {
        logger::info("Loading SL Aroused settings");
        auto settings = Settings::GetSingleton();

        CSimpleIniA ini;
        ini.SetUnicode();
        ini.LoadFile(R"(.\Data\SKSE\Plugins\OSLAroused_Settings.ini)");
        // const auto dataHandler = RE::TESDataHandler::GetSingleton();

        get_value<bool>(ini, settings->enabled, "Settings", "Enabled");
        logger::info("Loaded enabled: {}", settings->enabled);
        get_value<float>(ini, settings->updateRate, "Settings", "UpdateRate");
