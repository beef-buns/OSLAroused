...
RE::BSEventNotifyControl RuntimeEvents::OnEquipEvent::ProcessEvent(const RE::TESEquipEvent *equipEvent,
    ...
    ActorStateManager::GetSingleton()->UpdateWornKeywordCache(static_cast<RE::Actor *>(equipActor), armor,
                                                              equipEvent->equipped);
    ...
    if (armor->HasPartOf(RE::BGSBipedObjectForm::BipedObjectSlot::kBody)) {
        ...
        ActorStateManager::GetSingleton()->ActorNakedStateChanged(static_cast<RE::Actor *>(equipEvent->actor.get()),
                                                                  !equipEvent->equipped);
        ...
        for (uint32_t i = 0; i < keywordForm->numKeywords; i++) {
            if (keywordForm->keywords[i]->formEditorID == "ArmorCuirass") {
                ActorStateManager::GetSingleton()->ActorNakedStateChanged(
                        static_cast<RE::Actor *>(equipEvent->actor.get()), !equipEvent->equipped);
            }
        }
    }

    //Changed equipped armor so update devices
    DevicesIntegration::GetSingleton()->ActiveEquipmentChanged(static_cast<RE::Actor *>(equipEvent->actor.get()),
                                                               equipmentForm, equipEvent->equipped);
    return RE::BSEventNotifyControl::kContinue;
}
...
void WorldChecks::ArousalUpdateLoop() {
    ...
    float elapsedGameTimeSinceLastCheck = std::clamp(
            curHours - WorldChecks::ArousalUpdateTicker::GetSingleton()->LastUpdatePollGameTime, 0.f, 1.f);
...
std::vector<RE::Actor *> GetNearbySpectatingActors(RE::Actor *source, float radius) {
    ...
    Utilities::World::ForEachReferenceInRange(source, radius, [&](RE::TESObjectREFR *ref) {
        auto refBase = ref->GetBaseObject();
        auto actor = ref->As<RE::Actor>();
        if (actor && actor != source && !actor->IsDisabled() &&
            (ref->Is(RE::FormType::NPC) || (refBase && refBase->Is(RE::FormType::NPC)))) {
            //If Actor is super close or detects the source, increase arousal
            if (sourceLocation.GetSquaredDistance(ref->GetPosition()) < forceDetectDistance ||
                (actor->RequestDetectionLevel(source, RE::DETECTION_PRIORITY::kNormal) > 0) || actor->IsPlayer()) {
